...
async function handleCreatePayment(request, env) {
  ...
  // Two ways a customer can pay:
  //  - "crypto": they pay directly in a stablecoin/crypto (checkout/payin/crypto)
  //  - "fiat":   they pay via bank/card in their own currency, which Noah
  //              converts behind the scenes (checkout/payin/fiat) — this is
  //              the "USD in, EUR out" cross-border rail.
  ...
  // NOTE: amount is stored at face value regardless of currency — for a
  // merchant using multiple currencies the estimated balance below would
  // mix them together. Fine for this MVP/single-currency testing; would
  // need per-currency totals before using with multiple currencies.
  await env.DIGIPAGOS_CUSTOMERS.put(
...
async function handleCreatePayout(request, env) {
  ...
  // withdrawing and how much. CryptoAuthorizedAmount is the max crypto
  // Noah may debit for this payout — using the same 1:1 stablecoin≈USD
  // approximation as elsewhere in this MVP.
  const payload = {
