 ...
 *  - Sending a payment to a third-party recipient (e.g. paying a supplier)
...
const NONCUSTODIAL = {
  ...
  sendPaymentDisclaimer:
    "This sends a payout instruction to Noah for a recipient you registered. DigiPagos never holds or forwards the funds itself — Noah settles directly to the recipient's bank account.",
};
...
async function handleCreatePayout(request, env) {
  ...
  const amountStr = Number(amount).toFixed(2);
  const noahCustomerId = toNoahCustomerId(email);
  const externalId = crypto.randomUUID(); // max 36 chars allowed by Noah
...
/* ------------------------------------------------------------------ */
/*  5d. Register a third-party payout recipient (e.g. a supplier)      */
/*      NOTE: Noah's exact field names for this endpoint are our best  */
/*      guess from their API reference listing — confirm/adjust once   */
/*      tested against sandbox, the same way we fixed earlier          */
/*      endpoints (check the error response, adjust field names).      */
/* ------------------------------------------------------------------ */
async function handleCreateRecipient(request, env) {
  ...
  const { recipientName, country, currency, accountNumber, bankCode, accountHolderAddress } =
    await request.json();
  ...
  // common field name candidates (same pattern as HostedURL earlier).
  ...
  return jsonResponse({
    ...
    note: "Bank details are forwarded to Noah to register the recipient. DigiPagos does not keep account numbers in its ledger.",
  });
}

/* ------------------------------------------------------------------ */
/*  5e. List saved recipients (for the "Send payment" dropdown)        */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  5f. Send a payment to a saved third-party recipient (e.g. supplier)*/
...
async function handleSendPayment(request, env) {
  ...
  const { recipientId, amount, cryptoCurrency } = await request.json();
  ...
  const payload = {
    ...
    LineItems: [
      {
        Description: `Payment to recipient ${recipientId}`,
        Quantity: "1",
        UnitAmount: amountStr,
        TotalAmount: amountStr,
      },
...
function formatNoahError(noahData) {
  if (!noahData) return "";
  return noahData.message || noahData.Message || noahData.error || "";
}
