...
const NONCUSTODIAL = {
  ...
  sendPaymentDisclaimer:
    "This sends a payout instruction to Noah for a recipient you registered. DigiPagos never holds or forwards the funds itself — Noah settles directly to the recipient's bank account. DigiPagos is not the remitter of record and does not resolve bank rejects, recalls, or sanctions holds.",
};
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === "/api/noah/recipients" && request.method === "POST") {
        return await handleCreateRecipient(request, env);
      }

      if (path === "/api/noah/recipients" && request.method === "GET") {
        return await handleListRecipients(request, env);
      }

      if (path === "/api/noah/send-payment" && request.method === "POST") {
        return await handleSendPayment(request, env);
      }
...
/* ------------------------------------------------------------------ */
/*  6b. Register a third-party payout recipient (e.g. a supplier)      */
/*      NOTE: Noah's exact field names for this endpoint are our best  */
/*      guess from their API reference listing — confirm/adjust once   */
/*      tested against sandbox (read the real error, adjust), the same */
/*      pattern used throughout this integration.                     */
/* ------------------------------------------------------------------ */
async function handleCreateRecipient(request, env) {
  const email = await getSessionEmail(request, env);
  if (!email) {
    return jsonResponse({ error: "not_authenticated" }, 401);
  }
  ...
  const body = await request.json().catch(() => ({}));
  const { recipientName, country, currency, accountNumber, bankCode, accountHolderAddress } = body;

  if (!recipientName || !country || !currency || !accountNumber) {
    return jsonResponse(
      { error: "invalid_input", message: "recipientName, country, currency and accountNumber are required" },
      400
    );
  }

  const noahCustomerId = toNoahCustomerId(email);
  const badCurrency = currencyError(env, currency);
  if (badCurrency) return badCurrency;
  const blocked = await kycBlocked(env, noahCustomerId);
  if (blocked) return blocked;
  ...
  const noahRes = await fetch(`${noahBase}/customer-profile-payout-recipients`, {
    method: "POST",
  ...
  // Noah's response should include an ID for this recipient — try the
  // common field name candidates (same pattern used for HostedURL earlier).
  const recipientId = noahData.RecipientID || noahData.ID || noahData.Id;

  // Display metadata only — never persist account numbers or routing data.
  if (recipientId) {
    await env.DIGIPAGOS_CUSTOMERS.put(
      `merchant-recipient:${noahCustomerId}:${recipientId}`,
      JSON.stringify({
        recipientId,
        recipientName,
        country,
  ...
  return jsonResponse({
    ...
    custody: NONCUSTODIAL.model,
    processor: NONCUSTODIAL.processor,
    note: "Bank details are forwarded to Noah to register the recipient. DigiPagos does not keep account or routing numbers in its ledger.",
  });
}

/* ------------------------------------------------------------------ */
/*  6c. List saved recipients (for the "Send payment" dropdown)        */
/* ------------------------------------------------------------------ */
async function handleListRecipients(request, env) {
  const email = await getSessionEmail(request, env);
  if (!email) {
    return jsonResponse({ error: "not_authenticated" }, 401);
  }
  ...
  const noahCustomerId = toNoahCustomerId(email);
  const list = await env.DIGIPAGOS_CUSTOMERS.list({ prefix: `merchant-recipient:${noahCustomerId}:` });
  ...
  return jsonResponse({ ok: true, recipients: recipients.filter(Boolean) });
}

/* ------------------------------------------------------------------ */
/*  6d. Send a payment to a saved third-party recipient (e.g. supplier)*/
/*      NOTE: field names are our best guess — same "test, read the    */
/*      real error, adjust" pattern used throughout this integration.  */
/* ------------------------------------------------------------------ */
async function handleSendPayment(request, env) {
  const email = await getSessionEmail(request, env);
  if (!email) {
    return jsonResponse({ error: "not_authenticated" }, 401);
  }

  const { recipientId, amount, cryptoCurrency } = await request.json().catch(() => ({}));

  if (!recipientId || !amount || Number(amount) <= 0) {
    return jsonResponse({ error: "invalid_input", message: "recipientId and a positive amount are required" }, 400);
  }
  ...
  // Confirm this recipient actually belongs to the logged-in merchant
  // before sending anything — prevents paying someone else's recipient.
  const ownsRecipient = await env.DIGIPAGOS_CUSTOMERS.get(`merchant-recipient:${noahCustomerId}:${recipientId}`);
  if (!ownsRecipient) {
    return jsonResponse({ error: "forbidden", message: "Recipient does not belong to this account" }, 403);
  }
  ...
  const payload = {
    ...
    Nonce: crypto.randomUUID(),
    LineItems: [
      { Description: `Payment to recipient ${recipientId}`, Quantity: "1", UnitAmount: amountStr, TotalAmount: amountStr },
    ],
  };

  const noahRes = await fetch(`${noahBase}/third-party-payout`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Api-Key": env.NOAH_API_KEY },
    body: JSON.stringify(payload),
  });
  ...
  await env.DIGIPAGOS_CUSTOMERS.put(
    ...
    JSON.stringify({
      ...
      direction: "out",
      amount: Number(amountStr),
      currency: cryptoCurrency || "USDC_TEST",
      status: "Pending",
      type: "third_party_payout",
      recipientId,
      createdAt: new Date().toISOString(),
      custody: "noah",
    })
  ...
  return jsonResponse({
    ...
    custody: NONCUSTODIAL.model,
    processor: NONCUSTODIAL.processor,
    disclaimer: NONCUSTODIAL.sendPaymentDisclaimer,
  });
}
