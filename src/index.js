...
async function handleCreatePayment(request, env) {
  ...
  const { amount, cryptoCurrency, fiatCurrency, returnUrl, cancelUrl } = await request.json();
  ...
  // NOTE: field names below are our best guess from Noah's SDK docs for
  // POST /checkout/payin/crypto — confirm exact request/response shape
  // against the sandbox response the same way we did for onboarding
  // (check Network tab / Cloudflare logs) and adjust if Noah rejects it
  // or names the checkout URL field differently.
  const payload = {
    CustomerID: email,
    ...
    CancelURL: cancelUrl || "https://digipagos.io/dashboard.html",
  };
...
async function handleListTransactions(request, env) {
  ...
  const list = await env.DIGIPAGOS_CUSTOMERS.list({ prefix: `merchant-tx:${email}:` });
...
async function handleNoahWebhook(request, env) {
  ...
  // Transaction events: file under the merchant's history so the
  // dashboard can list it. NOTE: confirm the exact field Noah uses to
  // reference the merchant (CustomerID vs a nested field) once we see
  // a real Transaction event payload — adjust below if this key is wrong.
  if (event?.EventType === "Transaction" && event?.Data) {
    const merchantEmail = event.Data.CustomerID;
    const transactionId = event.Data.TransactionID || event.Data.ID;
    if (merchantEmail && transactionId) {
      await env.DIGIPAGOS_CUSTOMERS.put(
        `merchant-tx:${merchantEmail}:${transactionId}`,
        JSON.stringify(event.Data)
      );
    } else {
      console.warn("Transaction event missing CustomerID or TransactionID — check field names", JSON.stringify(event.Data));
    }
  }
